...
describe('Initializers', () => {
  describe('resolveSource', () => {
    ...
    beforeEach(() => {
      resolveStub = env.stub(resolve.MetadataResolver.prototype, 'resolveSource');
    ...
    it('should resolve source with a single file path', () => {
      ...
      expect(resolveStub.callCount).to.equal(1);
      expect(resolveStub.getCall(0).args[0]).to.deep.equal([fsPath]);
    });
    ...
    it('should resolve source with multiple file paths', () => {
      ...
      expect(resolveStub.callCount).to.equal(1);
      expect(resolveStub.getCall(0).args[0]).to.deep.equal([fsPath, fsPath2]);
    });
    ...
    it('should resolve source with options object', () => {
      ...
      expect(resolveStub.callCount).to.equal(1);
      expect(resolveStub.getCall(0).args[0]).to.deep.equal([fsPath]);
      expect(resolveStub.getCall(0).args[1]).to.deep.equal(resolved);
    });
