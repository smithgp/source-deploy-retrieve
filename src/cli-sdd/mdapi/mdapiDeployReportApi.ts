...
class MdDeployReportApi {
  ...
  constructor(
    org,
    pollIntervalStrategy?,
    private stashkey: string = Stash.Commands.MDAPI_DEPLOY
  ) {
    this.scratchOrg = org;
    ...
    //   format: `${
    //     stashkey.split('_')[0]
    //   } PROGRESS | {bar} | {value}/{total} Files`,
    //   barCompleteChar: '\u2588',
    //   barIncompleteChar: '\u2591',
    //   linewrap: true
    // });
  ...
  _printComponentFailures(result) {
    if (result.details && result.details.componentFailures) {
      ...
      this.logger.styledHeader(
        this.logger.color.red(`Component Failures [${failures.length}]`)
      );
  ...
  _printTests(result) {
    if (result.details && result.details.runTestResult) {
      if (result.details.runTestResult.failures) {
        ...
        const tests: Array<string> = _.chain(
          result.details.runTestResult.failures
        )
          .sortBy([
            function(o) {
        ...
        this.logger.styledHeader(
          this.logger.color.red(
            `Test Failures [${result.details.runTestResult.numFailures}]`
          )
        );
      ...
      if (result.details && result.details.runTestResult.successes) {
        ...
        this.logger.styledHeader(
          this.logger.color.green(
            `Test Success [${result.details.runTestResult.successes.length}]`
          )
        );
        this.logger.table(tests, {
          columns: [
      ...
      if (result.details && result.details.runTestResult.codeCoverage) {
        ...
        const coverage: Array<string> = _.chain(
          result.details.runTestResult.codeCoverage
        )
          .sortBy([
            function(o) {
          ...
          columns: [
            { key: 'name', label: 'Name' },
            {
              ...
              format: (numLocations, row) => {
                ...
                const numLocationsNotCovered: number = parseInt(
                  row.numLocationsNotCovered
                );
      ...
      if (
        ...
        this._log('');
        this._log(
          `Total Test Time:  ${result.details.runTestResult.totalTime}`
        );
      }
    }
  }
  ...
  _print(options, result) {
    if ((this.loggingEnabled || options.source) && !options.json) {
      ...
      if (result.timedOut) {
        ...
        this._printOldOutput(result);
        this._log(
          messages.getMessage('mdDeployCommandCliWaitTimeExceededError', [
            options.wait
          ])
        );
        return result;
    ...
    if (result.completedDate) {
      ...
      if (options.checkonly) {
        ...
        if (!result.numberComponentErrors) {
          const coreMessages = MessagesCore.loadMessages(
  ...
  report(options): BBPromise {
    ...
    this.loggingEnabled =
      options.source ||
      options.verbose ||
      (!options.json && !options.disableLogging);
  ...
  async validate(context): BBPromise {
    ...
    const validWaitValue =
      !isNaN(+options.wait) && (+options.wait === -1 || +options.wait >= 0);
  ...
  _doDeployStatus(options): BBPromise {
    ...
    if (
      options.result &&
      options.wait == 0 &&
      !options.deprecatedStatusRequest
    ) {
