...
export class MdRetrieveApi {
  ...
  async retrieve(options: MdRetrieveOptions) {
    ...
    return retrievePromise
      .then(unpackagedJson =>
        this._formatRetrieveOptions(options, unpackagedJson)
      )
      ...
      .catch(err => {
        if (err.message.toLowerCase().includes('polling time out')) {
          const waitTime = options.wait
            ? options.wait
            : consts.DEFAULT_MDAPI_RETRIEVE_WAIT_MINUTES;
          throw almError('mdapiCliWaitTimeExceededError', [
            'retrieve',
            waitTime
          ]);
        } else {
  ...
  static _getPackageJson(mdApi, options) {
    ...
    return promise
      ...
      .then(unpackagedXml =>
        // convert to json
        xml2jsParseString(unpackagedXml, { explicitArray: false }).catch(
          err => {
            // wrap in SfdxError
  ...
  _formatRetrieveOptions(options, unpackagedJson) {
    ...
    retrieveOptions.apiVersion = Number(
      options.apiversion || this.force.config.getApiVersion()
    );
    ...
    if (options.packagenames) {
      ...
      retrieveOptions.packageNames = this._parsePackageNames(
  ...
  validate(context) {
    ...
    const willCreateManifestFromArtifact =
      options.sourcedir || (!options.packagenames && !options.unpackaged);
    ...
    if (!insideProjectWorkspace && willCreateManifestFromArtifact) {
      return BBPromise.reject(
        almError('mdRetrieveCommandCliInvalidProjectError')
      );
    }

    try {
      MetadataTransportInfo.validateExclusiveFlag(
        options,
        'sourcedir',
        'unpackaged'
      );
    ...
    if (
      options.apiversion &&
      (isNaN(+options.apiversion) ||
        +options.apiversion < 0 ||
        +options.apiversion > currentApiVersion)
    ) {
      return BBPromise.reject(
        almError(
    ...
    if (options.sourcedir) {
      ...
      validationPromises.push(
        this._validatePath(
          ...
          data => data.isFile(),
          () => BBPromise.resolve(),
          almError('InvalidArgumentFilePath', [
            'unpackaged',
            options.unpackaged
          ])
        )
  ...
  _validatePath(pathToValidate, validationFunc, successFunc, error) {
    return this._fsStatAsync(pathToValidate)
      ...
      .catch(err => {
        err =
          err.code === 'ENOENT'
            ? almError('PathDoesNotExist', pathToValidate)
            : err;
        return BBPromise.reject(err);
      });
  }
