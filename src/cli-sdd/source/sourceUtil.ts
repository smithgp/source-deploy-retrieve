...
/**
 * sObjects and standard fields aren't returned from SourceMember queries
...
export const getSourceMembersFromResult = async function(
  ...
  const allMembers: SourceMember[] = await maxRevision.queryAllSourceMembers();
  ...
  const successeNames: string[] = successes
  ...
  const diffedMembers = allMemberNames.filter(al => successeNames.includes(al));
