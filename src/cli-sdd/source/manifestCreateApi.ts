...
function generateMetadataManifestJson(
  packageName,
  typesAsKeyValuePairsArray,
  apiVersion
) {
...
function processMetadataFile(dir, file, childLogger, metadataRegistry) {
  ...
  if (fileInfo === null) {
    childLogger.info(
      `WARNING: Error parsing metadata file.  Ignoring - ${filePath}`
    );
  }
    ...
    .then(fileInfoArray => {
      ...
      fileInfoArray.forEach(fileInfo => {
        if (fileInfo.isDirectory) {
          ...
          fileInfo.name.endsWith(MetadataRegistry.getMetadataFileExt())
        ) {
...
function processMetadataDirectory(
  dir,
  childLogger,
  metadataRegistry,
  manifestcreate?
      ...
      BBPromise.map(entriesToBeProcessed.metadataFiles, file =>
        ...
        BBPromise.all([
          ...
          BBPromise.map(entriesToBeProcessed.dirs, childDir =>
            processMetadataDirectory(
              path.resolve(dir, childDir),
              childLogger,
              metadataRegistry,
              manifestcreate
            )
...
manifestCreate.prototype.execute = function execute(context) {
  ...
  const defaultSourceDirectory = !util.isNullOrUndefined(
    appConfig.defaultPackagePath
  )
    ? path.join(projectDirectory, appConfig.defaultPackagePath)
    : projectDirectory;
  const rootDirectory = optional
    .ofNullable(context.sourcedir)
    .orElse(defaultSourceDirectory);
  this.outputDirectory = optional
    .ofNullable(context.outputdir)
    .orElse(projectDirectory);
  const outputFile = path.resolve(this.outputDirectory, 'package.xml');
  const apiVersion = this.apiVersion;
  const orgApi = this.org;

  return this._validateDirectory(
    rootDirectory,
    almError('InvalidArgumentDirectoryPath', ['sourcedir', rootDirectory])
  )
    ...
    .then(() => {
      ...
      return processMetadataDirectory(
        rootDirectory,
        this.logger,
    ...
    .then(resultAsKeyValuePairs => {
      if (this.beforeManifestGenerationHook) {
        resultAsKeyValuePairs = this.beforeManifestGenerationHook(
          resultAsKeyValuePairs
        );
      }
      if (context.exclusions) {
        resultAsKeyValuePairs = resultAsKeyValuePairs.filter(element =>
          _.isNil(
            context.exclusions.find(
              exclusion =>
                exclusion.type === element.type &&
                exclusion.name === element.name
            )
          )
        );
      }
      const packageManifestJson = generateMetadataManifestJson(
        context.packageName,
        resultAsKeyValuePairs,
        apiVersion
      );
...
manifestCreate.prototype.createManifest = function(
  context,
  packageName,
  typeFullNamePairs
) {
  const outputDir = optional
    .ofNullable(context.outputdir)
    .orElse(this.config.getProjectPath());
  const outputFile = path.resolve(
    outputDir,
    optional.ofNullable(context.outputfile).orElse('package.xml')
  );
 ...
 */
manifestCreate.prototype.createManifestForMdapiPackage = function(
  context,
  mdapiPackage,
  metadataRegistry
) {
  ...
  return createOutputXmlManifestFile(
    outputFile,
    mdapiPackage.getPackage(metadataRegistry)
  );
};
