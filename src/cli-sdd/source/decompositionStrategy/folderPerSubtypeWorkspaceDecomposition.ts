  ...
  getDecomposedSubtypeDirFromAnnotation(
    ...
    const key =
      aggregateFullName === annotation.name
        ? SourceLocations.getFilePathKey(metadataType, aggregateFullName)
        : SourceLocations.getFilePathKey(metadataType, `${aggregateFullName}.${annotation.name}`);
    const filePathsIndex = SourceLocations.filePathsIndex;
    const filePath = filePathsIndex.get(key);
    if (filePath) {
      return path.dirname(filePath);
    } else if (filePathsIndex.has(aggregateFullName)) {
      return path.join(path.dirname(filePathsIndex.get(aggregateFullName)), decomposedSubtypeConfig.defaultDirectory);
    } else {
      return null;
    }
  }
