...
const KeychainPromises = {
  ...
  migrate(oldKeychain, newKeychain, service, account) {
    return KeychainPromises.get(oldKeychain, service, account).then(
      passwordResult =>
        KeychainPromises.set(
          newKeychain,
          service,
          account,
...
class Crypto {
  ...
  constructor(_keychainImpls = keyChainImpls) {
    ...
    logger.debug(
      `process.env.SFDX_DISABLE_ENCRYPTION: ${
        process.env.SFDX_DISABLE_ENCRYPTION
      }`
    );

    this.enableTokenEncryption =
      _.toUpper(process.env.SFDX_DISABLE_ENCRYPTION) !== 'TRUE';
  }
    ...
    return this.keychainImpls
          ...
          .catch(err => {
            ...
            if (err.name === 'PasswordNotFound') {
              ...
              if (retryStatus === 'KEY_SET') {
                ...
                logger.debug(
                  'password not found in keychin attempting to created one and re-init.'
                );
              }

              const key = crypto.randomBytes(Math.ceil(16)).toString('hex');
              // Create a new password in the KeyChain.
              return KeychainPromises.set(
                keychainImpl,
                KEY_NAME,
                ACCOUNT,
                key
      ...
      .catch(err => {
        if (retryStatus === 'MIGRATED') {
          logger.debug(
            'The key was successfully migrated but an error occured getting the keychain on retry'
          );
          throw err;
        }
        ...
        if (err.name === keyChainImpls.DEPRECATED_KEYCHAIN) {
          ...
          return KeychainPromises.migrate(
            err.fromKeychain,
            err.toKeychain,
            KEY_NAME,
            ACCOUNT
  ...
  decrypt(text) {
    ...
    try {
      ...
      if (os.platform() === 'darwin' && !srcDevUtil.useGenericUnixKeychain()) {
        e.message += Messages.loadMessages(
          '@salesforce/source-deploy-retrieve',
          'crypto'
        ).getMessage('MacKeychainOutOfSync');
      }
