...
const describeMetadataResponse = path.join(
  __dirname,
  '..',
  '..',
  '..',
...
Force.prototype.generateCodeChallenge = function() {
  this.codeVerifier = srcDevUtil.base64UrlEscape(
    crypto.randomBytes(Math.ceil(128)).toString('base64')
  );
};
...
Force.prototype.authorize = function(oauthConfig) {
  ...
  return BBPromise.resolve(connection.authorize(oauthConfig.authCode))
    .then(() =>
      connection.query(
        `SELECT Username FROM User WHERE Id = '${connection.userInfo.id}'`
      )
    )
    .then(response => ({
    ...
    .finally(() => {
      if (this.codeVerifier) {
        jsforce.OAuth2.prototype['_postParams'] = jsforceOauth2PostParams.bind(
          connection.oauth2
        );
      }
    });
...
Force.prototype.jwtAuthorize = function(oauthConfig) {
  return fsReadFile(oauthConfig.privateKeyFile, 'utf8')
    ...
    .then(token =>
      BBPromise.resolve(_jwtAuthorize.call(this, token, oauthConfig.loginUrl))
    )
    .catch(error => {
...
Force.prototype.saveOrgAuthData = function(
  ...
  thisLogger.debug(
    `Authenticated new org: ${authObject.orgId} for user ${
      authObject.username
    } with authcode.`
  );
...
Force.prototype.authorizeAndSave = function(
  oauthConfig,
  orgApi,
  saveAsDefault,
  ...
  if (!_.isNil(oauthConfig.privateKeyFile)) {
    ...
    _.isNil(oauthConfig.authCode) &&
    !_.isNil(oauthConfig.refreshToken)
  ) {
  ...
  return promise.then(authObject => {
    if (this.logger.getEnvironmentMode().isDemo()) {
      const query = `SELECT TrialExpirationDate FROM Organization WHERE Id = \'${
        authObject.orgId
      }\'`;
      ...
      return this.query(orgApi, query).then(result => {
        const trialExpirationDate =
          result && result.records.length === 1
...
Force.prototype.create = function(orgApi, sobjectName, sobject) {
  ...
  return this._getConnection(orgApi, this.config).then(connection =>
    _create(sobjectName, sobject, connection)
  );
};
...
Force.prototype.update = function(orgApi, sobjectName, sobject) {
  ...
  return this._getConnection(orgApi, this.config).then(connection =>
    connection.sobject(sobjectName).update(sobject)
  );
};
...
Force.prototype.delete = function(orgApi, sobjectName, id) {
  ...
  return this._getConnection(orgApi, this.config).then(connection =>
    connection.sobject(sobjectName).delete(id)
  );
};
...
Force.prototype.describe = function(orgApi, sobjectName) {
  ...
  return this._getConnection(orgApi, this.config).then(connection =>
    connection.sobject(sobjectName).describe()
  );
};
...
Force.prototype.getOrgFrontDoor = function(orgApi, tryRefresh = true) {
  ...
  return (tryRefresh ? this.describeData(orgApi) : BBPromise.resolve())
    ...
    .then(
      orgData =>
        `${_.trimEnd(orgData.instanceUrl, '/')}/secur/frontdoor.jsp?sid=${
          orgData.accessToken
        }`
    );
};
...
Force.prototype.retrieve = function(orgApi, sobjectName, id) {
  ...
  return this._getConnection(orgApi, this.config).then(connection =>
    connection.sobject(sobjectName).retrieve(id)
  );
};
...
Force.prototype.getAsyncTestResults = function(
  orgApi,
  apexTestQueueItemIds,
  chunkSize = consts.DEFAULT_CHUNKSIZE
) {
  ...
  const apexTestBatches =
    apexTestQueueItemIds.length > chunkSize
      ? _.chunk(apexTestQueueItemIds, chunkSize)
      : [apexTestQueueItemIds];
...
Force.prototype.toolingFind = function(
  orgApi,
  sobjectName,
  conditions,
  fields
...
Force.prototype.connectApiGet = async function(orgApi, resource) {
  ...
  const connection = await getCoreConnection(
    orgUserName,
    orgUserName ? null : orgApi.authConfig
  );
    ...
    .then(
      url =>
        new BBPromise((resolve, reject) => {
          ...
          form.append(
            'entity_content',
            JSON.stringify({ deployOptions: options }),
            {
...
Force.prototype.mdapiRetrieve = function(orgApi, options) {
  return this._getConnection(orgApi, this.config).then(connection =>
    connection.metadata.retrieve(options)
  );
};
...
Force.prototype.mdapiDescribe = function() {
  ...
  return fsReadFile(describeMetadataResponse, 'utf8')
    .then(JSON.parse)
    .then(res => {
      res.metadataObjects = _.isArray(res.metadataObjects)
        ? res.metadataObjects
        : [res.metadataObjects];
      res.metadataObjects = _.map(res.metadataObjects, mo => {
        if (mo.childXmlNames) {
          mo.childXmlNames = _.isArray(mo.childXmlNames)
            ? mo.childXmlNames
            : [mo.childXmlNames];
        }
...
Force.prototype.jwtConn = function(org, orgAuthConfig, connectData) {
  const refreshFn = function(conn, callback) {
    ...
    const oauthConfig = {
      loginUrl:
        orgAuthConfig.loginUrl ||
        org.config.getAppConfigIfInWorkspace().sfdcLoginUrl,
    ...
    return org.force
      ...
      .catch(err => {
        if (
          ...
          err.message.includes('Data Not Available')
        ) {
          return callback(
            almError(
...
Force.prototype.oauthConn = function(org, orgAuthConfig, connectData) {
  ...
  Object.assign(connectData, {
    oauth2: {
      loginUrl:
        orgAuthConfig.instanceUrl || this.config.getAppConfig().sfdcLoginUrl,
...
Force.prototype._getConnection = function(org, config, oauthConfig) {
  ...
  return (oauthConfig ? BBPromise.resolve(oauthConfig) : org.getConfig())
    .then(orgAuthConfig => {
      if (
        _.isNil(orgAuthConfig.refreshToken) &&
        _.isNil(orgAuthConfig.privateKey)
      ) {
