...
const orgSettingsMessages: Messages = Messages.loadMessages(
...
class SettingsGenerator {
  ...
  async extract(scratchDef, apiVersion?): Promise<void> {
    ...
    if (
      apiVersion >= 47.0 &&
      this.orgPreferenceSettingsMigrationRequired(scratchDef)
    ) {
  ...
  async migrate(scratchDef, apiVersion): Promise<void> {
    ...
    function lhccmdt(mdt) {
      ...
      return util.isNullOrUndefined(mdt)
        ? mdt
        : mdt.substring(0, 1).toLowerCase() + mdt.substring(1);
    }
  ...
  async extractAndMigrateSettings(scratchDef): Promise<void> {
    const oldScratchDef = JSON.stringify(
      { settings: scratchDef.settings },
      null,
      4
    );
    ...
    function storePrefs(data, pref, prefVal): boolean {
      ...
      if (OrgPrefRegistry.isMigrationDeprecated(orgPrefApi)) {
        ux.warn(
          `The setting "${pref}" is no longer supported as of API version 47.0`
        );
        return false;
      }
    ...
    var orgPreferenceSettings = getObject(
      scratchDef,
      'settings.orgPreferenceSettings'
    );
    ...
    for (var preference in orgPreferenceSettings) {
      if (
        storePrefs(
          this.settingData,
          preference,
          orgPreferenceSettings[preference]
        )
      ) {
        migrated = true;
      }
  ...
  private async writePackageFile(
    ...
    if (this.settingData) {
      ...
      packageContentInternals += util.format(
    ...
    if (this.objectSettingsData) {
      ...
      packageContentInternals += this.getTypeReferences(
        allRecTypes,
        'RecordType'
      );
      packageContentInternals += this.getTypeReferences(
    ...
    await fs.writeFile(
      packageFilePath,
      util.format(_packageFileContents, packageContentInternals, apiVersion)
    );
  }
  ...
  private async writeObjectSettingsIfNeeded(
    ...
    if (this.objectSettingsData) {
      ...
      for (const item of Object.keys(this.objectSettingsData)) {
        ...
        const fileContent = this._createObjectFileContent(
          this.cap(item),
          value,
          allRecTypes,
          allBps
        ...
        await fs.writeFile(
          path.join(objectsDir, this.cap(item) + '.object'),
          fileContent
        );
      }
    }
  ...
  private async writeSettingsIfNeeded(settingsDir: string) {
    if (this.settingData) {
      ...
      for (const item of Object.keys(this.settingData)) {
        ...
        await fs.writeFile(
          path.join(settingsDir, fname + '.settings'),
          fileContent
        );
      }
    }
...
`;
    ...
    if (sharingModel) {
      res +=
        '    <sharingModel>' + this.cap(sharingModel) + '</sharingModel>\n';
    }
