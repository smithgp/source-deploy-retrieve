...
export class RetrieveOperation extends MetadataApiOperation<RetrieveResult, SourceRetrieveResult> {
  private mdPackage: MetadataPackage;
  private options?: RetrieveOptions;

  constructor(connection: Connection, mdPackage: MetadataPackage, options?: RetrieveOptions) {
    super(connection);
  ...
  protected async pre(): Promise<{ id: string }> {
    ...
    return this.connection.metadata.retrieve({
      apiVersion: this.mdPackage.apiVersion,
      unpackaged: this.mdPackage.getObject().Package,
    });
  }
    ...
    if (result.messages) {
      ...
      for (const retrieveMessage of messages) {
        ...
        if (matches && Array.isArray(matches)) {
          ...
          failedComponent = this.mdPackage
            .getComponents()
            .getAll()
  ...
  private calculateSuccess(
    ...
    return (
      (retrieveResult.status === RequestStatus.Succeeded &&
        this.mdPackage.getComponents().size === components.length &&
